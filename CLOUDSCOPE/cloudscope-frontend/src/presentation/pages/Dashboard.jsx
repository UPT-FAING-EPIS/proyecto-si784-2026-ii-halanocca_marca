    ...
    <div className="flex min-h-screen bg-slate-50 text-slate-800 font-sans">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-5">
          ...
          <div className="space-y-1">
            ...
            {/* Guía para Principiantes (Tutorial Paso a Paso) */}
            <button
              onClick={() => setActiveNavTab('guide')}
              className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs font-bold transition-all ${
                activeNavTab === 'guide'
                  ? 'bg-[#0084ff] text-white shadow-sm'
                  : 'text-slate-700 hover:bg-slate-100'
            ...
            >
              ...
              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${activeNavTab === 'guide' ? 'bg-white/20 text-white' : 'bg-emerald-50 text-emerald-700'}`}>
                Tutorial
              </span>
            </button>

            {/* Panel Administrador (solo visible para admins) */}
            {user?.role === 'admin' && (
              <button
                onClick={() => navigate('/admin')}
                className="w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs font-bold transition-all text-red-600 hover:bg-red-50"
              >
                <div className="flex items-center gap-2.5">
                ...
                <span className="text-[9px] font-black px-1.5 py-0.5 rounded uppercase tracking-wider bg-red-100 text-red-600">
                  ADMIN
                </span>
              </button>
            )}
      ...
      <div className="flex-1 min-w-0 flex flex-col min-h-screen overflow-y-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5 pl-2 border-l border-slate-200">
              ...
              <span className="text-xs font-bold text-amber-600">
                ...
                {activeNavTab === 'guide' && 'Guía para Principiantes'}
              </span>
            </div>
        ...
        <main className="max-w-7xl w-full mx-auto px-6 md:px-8 py-8 space-y-8 flex-1">
          ...
          {/* ────────────────────────────────────────────────────────────────
              GUÍA PASO A PASO PARA PRINCIPIANTES (activeNavTab === 'guide')
             ──────────────────────────────────────────────────────────────── */}
          {activeNavTab === 'guide' && (
            <div className="animate-in fade-in duration-200">
              <Guide onBack={() => setActiveNavTab('overview')} />
            </div>
          )}
          ...
          {activeNavTab === 'overview' && (
            <div className="space-y-10 animate-in fade-in duration-200">
              ...
              {/* Banner de Guía para Principiantes */}
              <div className="p-5 rounded-2xl bg-gradient-to-r from-blue-600 via-indigo-600 to-sky-600 text-white shadow-md flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-start gap-3.5">
                  <div className="p-2.5 rounded-xl bg-white/20 backdrop-blur-xs text-white shrink-0 mt-0.5">
                    <SparkleIcon className="w-6 h-6" />
                  </div>
                  <div>
                    <span className="inline-block text-[10px] font-extrabold uppercase tracking-wider bg-white/25 px-2 py-0.5 rounded-full mb-1">
                      NUEVO • GUÍA PASO A PASO
                    </span>
                    <h3 className="text-base md:text-lg font-black tracking-tight">
                      ¿Eres nuevo en CloudScope o quieres aprender desde cero?
                    </h3>
                    <p className="text-xs text-blue-100 max-w-xl leading-relaxed mt-0.5">
                      Aprende los conceptos de la nube, cómo crear un proyecto paso a paso, arrastrar servicios, calcular costos FinOps y probar nuestro mini simulador interactivo.
                    </p>
                  </div>
                </div>
                ...
                <button
                  onClick={() => setActiveNavTab('guide')}
                  className="px-4 py-2.5 rounded-xl text-xs font-black text-blue-900 bg-white hover:bg-blue-50 transition-all shadow-md shrink-0 flex items-center justify-center gap-2 self-start md:self-center cursor-pointer"
                >
                  <span>Abrir Guía y Tutorial</span>
                  <ArrowRightIcon className="w-3.5 h-3.5" />
                </button>
              </div>
